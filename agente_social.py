...
def subir_imagen_a_r2(image_bytes, filename=None):
    ...
    s3 = boto3.client("s3", endpoint_url=endpoint, aws_access_key_id=CF_R2_ACCESS_KEY,
        aws_secret_access_key=CF_R2_SECRET_KEY, region_name="auto")
    s3.put_object(Bucket=CF_R2_BUCKET, Key=filename, Body=image_bytes, ContentType="image/png")
    public_url = "{}/{}".format(CF_R2_PUBLIC_URL, filename)
    print("  R2 upload OK: {}".format(public_url))
    return public_url
